...
import { FaSpinner, FaPassport, FaGlobe, FaClock, FaCalendarAlt } from 'react-icons/fa';
...
export default function VisaPayment() {
    ...
    useEffect(() => {
        ...
        setVisaData(JSON.parse(savedVisa));
    }, [navigate]);
        ...
        try {
            const response = await fetch('https://alfalahpayemnt-production.up.railway.app/api/alfa/pay', {
                ...
                body: JSON.stringify({
                    ...
                    amount: visaData.totalFee,
                    transactionType: paymentType,
                    isRedirectionRequest: "1",
                    customerEmail: visaData.email,
                    customerName: visaData.applicantName,
                    customerMobile: visaData.phone,
                    paymentFor: 'visa' // Identifier for visa payment
                })
            });
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-6 font-sans antialiased selection:bg-blue-100">
            ...
            <div className="w-full max-w-6xl mb-8 flex justify-between items-center">
                ...
                >
                    <MdArrowBack size={20} /> Back to Application
                </button>
            ...
            <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                {/* LEFT SIDE: VISA APPLICATION SUMMARY */}
                <motion.div
                ...
                >
                    <div className="bg-white rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                        <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 p-8 text-white">
                            <p className="text-emerald-200 text-xs font-black uppercase tracking-[0.2em] mb-2">Visa Application</p>
                            <h3 className="text-2xl font-black flex items-center gap-2">
                                <FaPassport /> {visaData.country}
                            </h3>
                        </div>
                        <div className="p-8 space-y-5">
                            <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">Applicant</span>
                                <span className="text-slate-800 font-black">{visaData.applicantName}</span>
                            </div>
                            <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">Visa Type</span>
                                <span className="text-slate-800 font-black">{visaData.visaType}</span>
                            </div>
                            <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">Processing</span>
                            ...
                            <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">Validity</span>
                                <span className="text-slate-800 font-black flex items-center gap-1">
                            ...
                            <div className="mt-8 pt-6">
                                <div className="bg-emerald-50 rounded-2xl p-6 text-center">
                                    <p className="text-emerald-600 text-xs font-black uppercase tracking-widest mb-1">Total Fee</p>
                                    <p className="text-4xl font-black text-emerald-700">PKR {visaData.totalFee.toLocaleString()}</p>
                                    {visaData.urgentProcessing && (
                                        <p className="text-xs text-slate-500 mt-2">
                                            Visa Fee: PKR {visaData.visaFee.toLocaleString()} + Urgent: PKR {visaData.urgentFee.toLocaleString()}
                                        </p>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded-[32px] shadow-2xl shadow-blue-900/10 border border-slate-100 overflow-hidden">
                        ...
                        <div className="p-10 space-y-8">
                            ...
                            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center">
                                ...
                                <span className="text-xl font-black text-slate-800">PKR {visaData.totalFee.toLocaleString()}</span>
                            </div>
