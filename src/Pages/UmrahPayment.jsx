...
import {
    ...
    MdArrowForward, MdVerifiedUser, MdCheckCircle, MdErrorOutline,
    MdLock, MdShield, MdArrowBack
} from 'react-icons/md';
import { FaSpinner, FaKaaba, FaHotel, FaCalendarAlt } from 'react-icons/fa';
import { useNavigate } from 'react-router-dom';

export default function UmrahPayment() {
    ...
    const [umrahData, setUmrahData] = useState(null);

    useEffect(() => {
        const saved = localStorage.getItem('pending_umrah_payment');
    ...
    const handlePayment = async () => {
        ...
        try {
            const response = await fetch('https://alfalahpayemnt-production.up.railway.app/api/alfa/pay', {
                ...
                body: JSON.stringify({
                    transactionId: `UMRAH-${Date.now()}`,
                    amount: umrahData.paymentAmount,
                    transactionType: paymentType,
                    isRedirectionRequest: "1",
                    customerEmail: umrahData.email,
                    customerName: umrahData.applicantName,
                    customerMobile: umrahData.phone,
                    paymentFor: 'umrah' // Identifier for umrah payment
                })
            });
            ...
            if (data.success) {
                ...
                setRedirectForm({
                    action: data.data.paymentUrl,
                    fields: data.data.paymentFields
                });
            } else {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-6 font-sans antialiased selection:bg-blue-100">
            ...
            <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                {/* LEFT SIDE: UMRAH REQUEST SUMMARY */}
                <motion.div
                ...
                >
                    <div className="bg-white rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                        <div className="bg-gradient-to-br from-blue-600 to-blue-700 p-8 text-white">
                            <p className="text-blue-200 text-xs font-black uppercase tracking-[0.2em] mb-2">Umrah Package</p>
                            <h3 className="text-2xl font-black flex items-center gap-2">
                                <FaKaaba /> {umrahData.applicantName}
                            </h3>
                        </div>
                        <div className="p-8 space-y-5">
                            <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">Request #</span>
                                <span className="text-slate-800 font-black font-mono">{umrahData.requestNumber}</span>
                            </div>
                            <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">Hotel</span>
                                <span className="text-slate-800 font-black flex items-center gap-1 text-right">
                                    <FaHotel className="text-blue-600 shrink-0" />
                                    {umrahData.hotel}
                                </span>
                            </div>
                            {(umrahData.checkIn || umrahData.checkOut) && (
                                <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                    <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">Stay</span>
                            ...
                            {umrahData.paymentNote && (
                                <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
                                    <p className="text-blue-800 font-bold text-sm">Note from our team:</p>
                                    <p className="text-blue-700 text-sm mt-1">{umrahData.paymentNote}</p>
                                </div>
                            )}

                            <div className="mt-8 pt-6">
                                <div className="bg-blue-50 rounded-2xl p-6 text-center">
                                    <p className="text-blue-600 text-xs font-black uppercase tracking-widest mb-1">Amount Due</p>
                                    <p className="text-4xl font-black text-blue-700">PKR {Number(umrahData.paymentAmount).toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="bg-blue-600 rounded-3xl p-6 text-white flex items-center gap-4 shadow-lg shadow-blue-200">
                        <div className="bg-blue-500 p-3 rounded-2xl"><MdVerifiedUser size={30} /></div>
                        <div>
                            <p className="font-black text-lg">Guaranteed Safe</p>
                            <p className="text-blue-100 text-sm">Your information is fully encrypted.</p>
                        </div>
                    </div>
                </motion.div>
                    ...
                    <div className="bg-white rounded-[32px] shadow-2xl shadow-blue-900/10 border border-slate-100 overflow-hidden">
                        ...
                        <div className="p-10 space-y-8">
                                ...
                                {methods.map((m) => {
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            className={`flex items-center gap-5 p-6 rounded-2xl border-2 transition-all text-left ${active ? 'border-[#0052CC] bg-blue-50/30' : 'border-slate-100 hover:border-slate-200'
                                                }`}
                                        >
                            ...
                            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center">
                                ...
                                <span className="text-xl font-black text-slate-800">PKR {Number(umrahData.paymentAmount).toLocaleString()}</span>
                            </div>
                            ...
                            <AnimatePresence>
                                {status.message && (
                                    <motion.div
                                        ...
                                        animate={{ opacity: 1, y: 0 }}
                                        exit={{ opacity: 0 }}
                                        className={`p-5 rounded-2xl flex items-center gap-4 font-bold border ${status.type === 'error' ? 'bg-rose-50 text-rose-700 border-rose-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'
                                            }`}
                                    >
